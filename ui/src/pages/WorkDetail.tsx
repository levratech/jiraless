...
function cfg() {
	return (window as any).__JIRALESS__ || { owner: '', repo: '', branch: 'main' }
}
function qsToObj(s: string) {
	const p = new URLSearchParams(s)
	return Object.fromEntries(p.entries()) as Record<string, string>
}
...
export function WorkDetail() {
	...
	const [pat, setPat] = useState<string>(
		localStorage.getItem('JIRALESS_PAT') || ''
	)
					...
					if (hit?.file) {
						...
						if (hit.pr_number) {
							setPrData({
								...
								pr_state: hit.pr_state,
								checks_state: hit.checks_state,
								updated_at: hit.updated_at,
							})
						}
						return toRepoRel(hit.file)
					}
				}
			} catch {}
	...
	async function proposeTransition() {
		if (!id || !next) {
			setMsg('Select a next state.')
			return
		}
		if (!pat) {
			setMsg('Provide a GitHub PAT.')
			return
		}
	...
	const copyLink = () => {
		const url = `${window.location.origin}/work/${encodeURIComponent(
			id || ''
		)}${location.search}`
		...
		<div style={{ maxWidth: 800, margin: '0 auto', padding: 20 }}>
				...
				<div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
					...
					<div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
						...
						<Badges
							...
							pr_state={prData.pr_state as any}
							checks_state={prData.checks_state as any}
			...
			>
				<div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
					...
					{msg && (
						<span
							style={{ color: msg.includes('Error') ? '#dc2626' : '#059669' }}
						>
							{msg}
						</span>
					)}
