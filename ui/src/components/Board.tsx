...
import { indexBoard, filterAndSearch, getUniqueValues, Filters, WorkItem, highlightText } from '../lib/search'
...
export function Board() {
  ...
  const filteredItems = useMemo(() => filterAndSearch(allItems, filters), [allItems, filters])
  ...
  const availableTypes = useMemo(() => getUniqueValues(allItems, 'type'), [allItems])
  const availableAssignees = useMemo(() => getUniqueValues(allItems, 'assignees'), [allItems])
  ...
  const updateFilters = (updates: Partial<Filters>) => {
    ...
    if (newFilters.priorities.length) params.set('priority', newFilters.priorities.join(','))
    if (newFilters.assignees.length) params.set('assignee', newFilters.assignees.join(','))
    ...
    <div>
      ...
      <div style={{ marginBottom: 20, padding: 16, background: '#f8fafc', borderRadius: 8, border: '1px solid #e2e8f0' }}>
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
          <div>
            <label style={{ fontSize: 14, fontWeight: 500, marginRight: 8 }}>Search:</label>
            <input
              type="text"
              value={query}
          ...
          <div>
            <label style={{ fontSize: 14, fontWeight: 500, marginRight: 8 }}>Type:</label>
            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
              {availableTypes.map(type => (
                <button
                  ...
                  style={{
                    padding: '4px 8px',
                    border: `1px solid ${filters.types.includes(type) ? '#3b82f6' : '#d1d5db'}`,
                    background: filters.types.includes(type) ? '#dbeafe' : 'white',
                    borderRadius: 4,
                    fontSize: 12,
                    cursor: 'pointer'
                  }}
          ...
          <div>
            <label style={{ fontSize: 14, fontWeight: 500, marginRight: 8 }}>Priority:</label>
            <div style={{ display: 'flex', gap: 4 }}>
              {['p1', 'p2', 'p3', 'p4'].map(prio => (
                <button
                  ...
                  style={{
                    padding: '4px 8px',
                    border: `1px solid ${filters.priorities.includes(prio) ? '#3b82f6' : '#d1d5db'}`,
                    background: filters.priorities.includes(prio) ? '#dbeafe' : 'white',
                    borderRadius: 4,
                    fontSize: 12,
                    cursor: 'pointer'
                  }}
          ...
          <div>
            <label style={{ fontSize: 14, fontWeight: 500, marginRight: 8 }}>Assignee:</label>
            <input
              type="text"
              placeholder="Filter assignees..."
              onChange={e => updateFilters({ assignees: e.target.value ? [e.target.value] : [] })}
        ...
        {statuses.map((status) => (
          <div key={status} style={{ background: '#fafafa', border: '1px solid #eee', borderRadius: 12, padding: 12 }}>
            ...
            <div style={{ display: 'grid', gap: 8 }}>
              {filteredBoard[status]?.map(card => <Card key={card.id} {...card} query={query} />)}
            </div>
          </div>
        ))}
      </div>
...
function Card({ id, title, type, priority, assignees, file, query }: CardT & { query?: string }) {
  const rel = norm(file)
    ...
    <div style={{ position: 'relative' }}>
      <Link to={`/work/${encodeURIComponent(id)}${qs}`} style={{ textDecoration:'none', color:'inherit' }}>
        <div style={{ background:'white', border:'1px solid #e5e7eb', borderRadius:10, padding:10, boxShadow:'0 1px 2px rgba(0,0,0,0.04)' }}>
          ...
          <div style={{ fontWeight:600, marginBottom:6 }} dangerouslySetInnerHTML={{ __html: query ? highlightText(title, query) : title }} />
